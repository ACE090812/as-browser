-- Society bank bridge for the parts shop: PartsBank.balance / remove / add / name.
-- Auto-detects the banking resource. Every call is wrapped so a wrong export name prints a clear message
-- to the server console instead of breaking the shop. Set Config.parts.bank = 'custom' to use your own.

PartsBank = {}

local function cfg() return Config.parts or {} end
...
function PartsBank.name() pick(); return activeName end

--- Society balance (whole number) or nil when it cannot be read.
function PartsBank.balance(account)
    local d = pick()
...
function PartsBank.remove(account, amount, reason)
    local d = pick()
    if not d then return false end
    amount = math.floor(tonumber(amount) or 0)
    if amount <= 0 then return true end
    local bal = PartsBank.balance(account)
    if bal ~= nil and bal < amount then return false end
    local ok, res = pcall(d.remove, account, amount, reason or 'LS Parts Direct')
    ...
    if cfg().logTransactions ~= false and d.note then
        pcall(d.note, account, amount, reason or 'LS Parts Direct', false)
    end
    return true
end

--- Puts money back (refund). Returns true unless the bank raised an error.
function PartsBank.add(account, amount, reason)
    local d = pick()
    if not d then return false end
    amount = math.floor(tonumber(amount) or 0)
    if amount <= 0 then return true end
    local ok, err = pcall(d.add, account, amount, reason or 'LS Parts Direct refund')
    if not ok then log('refunding %s failed: %s', tostring(account), tostring(err)); return false end
    if cfg().logTransactions ~= false and d.note then
        pcall(d.note, account, amount, reason or 'LS Parts Direct refund', true)
    end
    return true
end
...
CreateThread(function()
    Wait(4000)
    local name = PartsBank.name()
    if name then
        log('society bank: %s', name)
    else
        log('^1no supported society bank is running^0 (Config.parts.bank = "%s"). Purchases are switched off until one is.', tostring(cfg().bank or 'auto'))
    end
end)
